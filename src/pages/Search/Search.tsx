...
const Search: FC = () => {
	...
	const updateFromURL = () => {
		...
		setTagFilterRows(prev => 
			prev.map(row => 
				excludedTags.includes(row.label) ? { ...row, checked: "excluded" } : row
			)
		);
		...
		if (searchTerm && searchTerm !== "enter search term or tag") {
			updatedFilters.push({ type: 'Search', name: searchTerm, check: 'checked' });
		}	
	...
	const constructSearchURL = () => {
		...
		// Add checked OS rows
		const checkedOSRows = OSFilterRows
			.filter(row => row.checked === "checked")
			.map(row => row.label);
	...
	const getPriceRangeNumber = (value: string) => {
		switch (value) {
			case "Free": return 0;
			case "Under $5.00": return 1;
			case "Under $10.00": return 2;
			case "Under $15.00": return 3;
			case "Under $20.00": return 4;
			case "Under $25.00": return 5;
			case "Under $30.00": return 6;
			case "Under $35.00": return 7;
			case "Under $40.00": return 8;
			case "Under $45.00": return 9;
			case "Under $50.00": return 10;
			case "Under $55.00": return 11;
			case "Under $60.00": return 12;
			case "Any Price": return 13;
			default: return 13;
		}
	};
